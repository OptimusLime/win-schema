...
function winSchema(winback, configuration)
{
  ...
  self.initialize = function(done)
  {
    ...
    self.log("VFunc: ", self.validateFunction);
    ...
    self.validateData = function(type, object, finished)
    {
      ...
      self.log('validate against: ', schema);
    	...
    	var result = self.validateFunction.apply(self.validator, [object, self.validator.getSchema(type), true, !self.allowUnknownKeys]);

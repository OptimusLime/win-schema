...
function extendAddSchema(self)
{
  ...
  //handle everything associated with adding a schema
       ...
       tJSON.forEach(function(node)
       {
        ...
        if(!self.allowAnyObjects && !this.isLeaf && !this.node["$ref"] && (this.node.type == "object" || this.node.type == "array"))
        {
          ...
          if(!hasNonKeywords){
            ...
            checkErrors[this.path.join(pathDelim)] = "AllowAnyObjects is off, therefore you cannot simple have an 'object' or 'array' type with no inner properties";
            checkErrors.length++;
          }
        }
    ...
    function parseSchemaReferences(schemaJSON)
    {
        ...
        if(this.node["$ref"] && !self.keywordRegExp.test(this.key))
       ...
       tJSON.forEach(function(node)
       {
          ...
          if(!this.isLeaf && this.key != "properties" && !Array.isArray(this.node))
          {
            ...
            if(!empty)
            {
              ...
              //we need to investigate the newly created properties object -- to continue down the rabbit whole
              this.keys.push('properties');
            }
